...
def evaluate(X,
    ...
    LDMAX6_GOAL = 17
    VCMAX6_GOAL = 200
    max_NOISE = np.zeros((n1, 1))
    max_WEMP = np.zeros((n1, 1))
    max_DOC = np.zeros((n1, 1))
    max_ROUGH = np.zeros((n1, 1))
    max_WFUEL = np.zeros((n1, 1))
    max_PURCH = np.zeros((n1, 1))
    min_RANGE = 200000 * np.ones((n1, 1))
    min_LDMAX = 200000 * np.ones((n1, 1))
    min_VCMAX = 200000 * np.ones((n1, 1))
    ...
    PFPF = \
\
        ((CSPD2 - CSPDM) ** 2 + (CSPD4 - CSPDM) ** 2 + (CSPD6 - CSPDM) ** 2 + (AR2 - ARM) ** 2 + (AR4 - ARM) ** 2 + \
\
         (AR6 - ARM) ** 2 + (SWEEP2 - SWEEPM) ** 2 + (SWEEP4 - SWEEPM) ** 2 + (SWEEP6 - SWEEPM) ** 2 + (
                 DPROP2 - DPROPM) ** 2 + \
\
         (DPROP4 - DPROPM) ** 2 + (DPROP6 - DPROPM) ** 2 + (WINGLD2 - WINGLDM) ** 2 + (WINGLD4 - WINGLDM) ** 2 + (
                 WINGLD6 - WINGLDM) ** 2 + \
\
         (AF2 - AFM) ** 2 + (AF4 - AFM) ** 2 + (AF6 - AFM) ** 2 + \
\
         (SEATW2 - SEATWM) ** 2 + (SEATW4 - SEATWM) ** 2 + (SEATW6 - SEATWM) ** 2 + (ELODT2 - ELODTM) ** 2 + (
                 ELODT4 - ELODTM) ** 2 + (ELODT6 - ELODTM) ** 2 + \
\
...
\
    ...
    max_NOISE = max_NOISE.reshape(1, -1)[0]
    ...
    max_WEMP = max_WEMP.reshape(1, -1)[0]
    ...
    max_WFUEL = max_WFUEL.reshape(1, -1)[0]
    ...
    max_PURCH = max_PURCH.reshape(1, -1)[0]
    ...
    min_RANGE = min_RANGE.reshape(1, -1)[0]
    ...
    min_LDMAX = min_LDMAX.reshape(1, -1)[0]
    ...
    min_VCMAX = min_VCMAX.reshape(1, -1)[0]
    ...
    # Individual constraint function values
    F = np.column_stack([f1, f2, f3, f4, f5, f6, f7, f8, f9, f10])
    G = np.column_stack([NOISE2_CV, WEMP2_CV, DOC2_CV, ROUGH2_CV, WFUEL2_CV, RANGE2_CV,
